  ...
  return (
    <div className='mt-20 pb-5'>
        <div className='text-4xl font-semibold mb-3
         text-mine-shaft-100 text-center'>Browse <span  className='text-bright-sun-400'>Job </span>Catagory</div>
         <div className='text-lg mb-10 mx-auto text-mine-shaft-300  w-1/2'> Explore diverse job oportunity tailored to your skills.
            ...
            jobCategories.map((category, index) => (
              <Carousel.Slide key={index}>
                <div className='flex flex-col items-center w-64 gap-2 border border-bright-sun-400 p-5 rounded-xl hover:cursor-pointer  hover:shadow-[0_0_5px_2px_black] !shadow-bright-sun-300 my-5 transition duration-300 ease-in-out'>
                  ...
                  <div className='text-mine-shaft-100 font-semibold text-xl'>{category.name}</div>
                  <div className='text-sm text-mine-shaft-300 text-center'>{category.description}</div>
                  <div className='text-bright-sun-300 text-lg'>{category.jobCount}</div>
                </div>
              </Carousel.Slide>
            ))
            }
